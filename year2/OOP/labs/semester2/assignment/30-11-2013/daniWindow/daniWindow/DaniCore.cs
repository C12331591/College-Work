...
namespace daniWindow
{
    ...
    public class DaniCore
    {
        ...
        public string Parse(string Sentence, bool answer)
        {
            ...
            /*If there were multiple spaces in a row, there will be empty elements in words[]. This loop removes any such elements.
            for (i = 0; i < words.Length; i++)
            {
                ...
                if (words[i] == "")
                {
                    string[] sanitised = new string[words.Length-1];
                    Array.Copy(words.Take(i-1).ToArray(), sanitised, 0);
                    Array.Copy(words.Skip(i+1).ToArray(), sanitised, i);
