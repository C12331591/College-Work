...
class Console extends Object
{
  ...
  Console (float rX, float rY, float Wid, float Hei, int[] Colour, String soundFile)
  {
    ...
    eject = new Eject(X + wid - (quarterWid / 2), Y + (hei/1.6), wid / 8, hei/7, buttonColour, "open.mp3");
    power = new Power(X + (quarterWid / 2), Y + (hei/1.6), wid / 8, hei/7, buttonColour, "power.mp3");
...
class Animated extends Object//this class serves as a base for any piece which can have any kind of animation.
{
  ...
  Animated(float rX, float rY, float Wid, float Hei, int[] Colour, String soundFile)
  {
    ...
    state = false;
    held = false;
    psFont = createFont("Typodermic - ZrnicRg-Regular.ttf", 32);
  }
  ...
  void drawPS(float x, float y, float Wid, float Hei, int[] gapColour, int[] textColour)
  {
    //yellow parts
    stroke(255, 204, 0);
    fill(255, 204, 0);
    ...
    //parts to create gaps
    stroke(gapColour[0], gapColour[1], gapColour[2]);
    fill(gapColour[0], gapColour[1], gapColour[2]);
    rect(161, 60, 8, 50);
    ellipse(165, 60, 8, 8);
...
class Eject extends Button
{
  Eject(float rX, float rY, float Wid, float Hei, int[] Colour, String soundFile)
  {
    super(rX, rY, Wid, Hei, Colour, soundFile);
  }
  
  Eject(float rX, float rY, float Wid, float Hei, int[] Colour)
  {
    super(rX, rY, Wid, Hei, Colour);
  }
  ...
  boolean draw()
  {
    boolean switched = super.draw();
    
    if (state)
    {
      drawSymbol(X, Y + (hei / 4.5));
    }
    else
    {
      drawSymbol(X, Y);
  ...
  void drawSymbol(float x, float y)
  {
    ...
    float xScale = (wid / 5) / 10.0f;
    float yScale = (hei / 5) / 7.0f;
...
{
  Power(float rX, float rY, float Wid, float Hei, int[] Colour, String soundFile)
  {
    super(rX, rY, Wid, Hei, Colour, soundFile);
  }
  
  Power(float rX, float rY, float Wid, float Hei, int[] Colour)
  {
    super(rX, rY, Wid, Hei, Colour);
  }
  ...
  void drawSymbol(float x, float y)
  {
    ...
    line(-8, -5, -8, 4);
    line(-4, 4, -1, -5);
    arc(6, -1, 7, 9, PI + (2 * (PI / 3)), (3 * PI) + (PI / 3));
    line(6, -7, 6, -2);
...
class Lid extends Animated
{
  ...
  void draw()
  {
    ...
    if (cy > (Y - (hei / 2)))//if the top of the lid is visible
    {
  ...
  void drawPS(float x, float y, float Wid, float Hei)
  {
    ...
    this.drawPS(x, y, Wid, Hei, colour, textColour);
...
void setup()
{
  ...
  console = new Console(100.0f, 225.0f, 600.0f, 300.0f, consoleColour, "running.mp3");
  tv = new TV(300.0f, 10.0f, 200.0f, 150.0f, tvColour);
}
