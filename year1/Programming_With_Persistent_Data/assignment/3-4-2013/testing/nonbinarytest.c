...
void add()
{
	...
	FILE *add;
	FILE *search;
	add=fopen("employees.dat", "a");
	search=fopen("employees.dat", "r");
	int found=0;
	int overwrite=0;
	char answer;
...
{
	FILE *check;
	check=fopen("employees.dat", "r");
	int created=1;
	
	if (check==NULL)
	{
		FILE *create;
		create=fopen("employees.dat", "w");
...
void update(int added, int marked)
{
	...
	FILE *fileaccess;
	fileaccess=fopen("employees.dat", "a");
	rewind(fileaccess);
