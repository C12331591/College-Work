...
void compact()
{
	FILE *old;
	FILE *new;
	old=fopen("employees.dat", "rb+");
	new=fopen("employeescomp.dat", "wb+");
	struct header htransfer;
	struct record rtransfer;
	
	fread(&htransfer, HEADERSIZE, 1, old);
	htransfer.totalrecs=htransfer.totalrecs-htansfer.delrecs;
	htansfer.delrecs=0;
	fwrite(&htranfer, HEADERSIZE, 1, new);
	
	while (fread(&rtransfer, RECSIZE, 1, old)
	{
		if (rtransfer.emp_num[0]!='*')
		{
			fwrite(&rtransfer, RECSIZE, 1, new);
		}
	}
