...
(deffunction add-widgets ()
  (?*c* add (new JLabel "This is: ") (BorderLayout.PAGE_START))
  (bind ?*t* (new JTextField))
  (bind ?*t2* (new JTextField))
...
(deffunction add-behaviours ()
  ...
  (?*t* addActionListener (implement ActionListener using
		(lambda (?name ?event)
		  (printout t "You chose: " (get ?*t* text) crlf)
		)
	)
  )
  
  (?*t2* addActionListener (implement ActionListener using
		(lambda (?name ?event)
		  (printout t "You chose: " (get ?*t2* text) crlf)
		)
	)
  )
)
